...
public class RedisService {
	  ...
	  @Path("/loadBalance")
		@Produces(MediaType.APPLICATION_JSON)
		public void callLoadBalance() {
		  RedisUtils.rebalanceTheSize();
		}
	  ...
	  @Path("/getAllNodes")
	  @Produces(MediaType.APPLICATION_JSON)
	  public ArrayList<NodeInfo> getAllNode(){
		 ...
		 for(String node:csvContents.split("\n")){
			  ...
			  String []childs = node.split(":");
			  if(childs.length>1){
					String child = childs[1];
	  ...
	  public NodeInfo postNewNode(NodeInfo node){
		  try {
		      File file = new File("/Users/Mana/Documents/workspace/RedisApi/src/redis-server-info.csv");
			  System.out.println("2222 name:"+file.getName()+"path"+file.getAbsolutePath());
		      if(!file.exists())
		       file.createNewFile();
		  ...
		  new RedisUtils().addServer(node.getIp()+":"+node.getPort());
		  return node;
	  }
	  ...
	  public Response deleteNode(@PathParam("port")int port){
		  String csvContents = readFromCSV();
			 ...
			 // File inputFile = new File("redis-server-info.csv");
				  File inputFile = new File("/Users/Mana/Documents/workspace/RedisApi/src/redis-server-info.csv");
