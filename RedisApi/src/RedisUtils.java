...
public class RedisUtils {
    ...
    static List<Jedis> redisNodes=null;
    ...
    public static List<Jedis> getRedisNodes(String results){
        ...
        redisNodes = new ArrayList<Jedis>();
    ...
    public static void rebalanceTheSize(){
    	redisNodes = getRedisNodes(new RedisUtils().readFromCSV());
        ...
        Long prevKey = (Long)Arrays.asList(hashServ.keySet().toArray()).get((hashServ.size()-1));
        for(Long key:hashServ.keySet()){
         if(hashServ.get(key).getClient().getPort()==maxNode.getClient().getPort()){
              replaceKey = key;
              break;
         }
         prevKey = key;
        ...
        hashServ.put(replaceKey, minNode);
	...
	public void deleteServer(Jedis ser) {
		...
		List<Long> deletdSlots = new ArrayList<Long>();
		SortedMap<Long, Jedis> tempHashServ = hashServ;
		for(Long key:tempHashServ.keySet()){
			Jedis value = tempHashServ.get(key);
			if(value.getClient().getPort()==ser.getClient().getPort()){
				deletdSlots.add(key);
			}
		}
		for(Long a : deletdSlots){
			hashServ.remove(a);
		}
	}
	
		//Method to add a server and migrate the data
		public void addServer(String servAdd) {
		
		
		System.out.println("Adding a new server node");
		String host;
		int port;

		host =servAdd.split(":")[0];
		System.out.println("**********"+host);
		port =Integer.parseInt(servAdd.split(":")[1]);
		Jedis jedis = new Jedis(host, port);
		redisNodes.add(jedis);
		for(int i=0;i<20;i++){
		byte[] hashBytes= getDigest().digest(ByteBuffer.allocate(4).putInt((jedis.toString()+String.valueOf(i)).hashCode()).array());
		...
		for(Long ke:hashServ.keySet()){
			System.out.println("Key : "+ke+" Server: "+hashServ.get(ke).getClient().getPort());
		}
		List<Object> serverSlot = Arrays.asList(hashServ.keySet().toArray());
		Jedis source = null;
		...
		Long current = (Long)serverSlot.get(i);
		Jedis temp = hashServ.get(current);
		System.out.println("Prev Port : "+prev);
		System.out.println("Current Port: "+temp.getClient().getPort());	
		boolean flag = false;

		if(temp.getClient().getPort()==jedis.getClient().getPort()){
		// If the same sever entry is next then loop till you get different server
			int ind = i;
			while(jedis.getClient().getPort()==hashServ.get((Long)serverSlot.get(ind)).getClient().getPort()){
				ind++;
				if(ind>=serverSlot.size()){
					ind=0;
					flag=true;
					
				}
			}
			source = hashServ.get((Long)serverSlot.get(ind));


		
		System.out.println("Migrate called : \n\t "+source.getClient().getPort()+" | "+jedis.getClient().getPort()+" | "+prev+" | "+current);
		if(flag){
			break;
		}
		migrate(source, jedis, prev, current);
		}
		prev = current;
